...
class MailRecipientCommandServiceTest {
    ...
    void addRecipient_whenAlreadyExists_throwsException() {
        Long mailId = 1L;
        Long userId = 100L;
        RecipientRequest request = new RecipientRequest(new UserId(userId), new CirculationTypeId(1L));
        ...
        when(hrServiceClient.getEmployee(userId)).thenReturn(Optional.of(emp));
