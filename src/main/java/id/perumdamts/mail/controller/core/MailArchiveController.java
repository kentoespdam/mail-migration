...
public class MailArchiveController {
    ...
    public PagedModel<ArchiveSummaryResponse> findForAdmin(@Valid @ParameterObject ArchiveSearchRequest request) {
        return new PagedModel<>(queryService.findForAdmin(request));
    }

    @GetMapping
    public PagedModel<ArchiveSummaryResponse> searchWithAcl(
            @AuthenticationPrincipal MailPrincipal principal,
            @Valid @ParameterObject ArchiveSearchRequest request,
            @RequestParam List<Long> positionIds) {
        return new PagedModel<>(queryService.searchWithAcl(request, positionIds));
    }
    ...
    @GetMapping("/report")
    @PreAuthorize("hasRole('ADMIN')")
    public PagedModel<ArchiveReportResponse> report(@Valid @ParameterObject ArchiveReportRequest request) {
        return new PagedModel<>(queryService.getReport(request));
    }
}
