...
1. **Plt model**: `employee.emp_pos_id` direct update vs tabel `employee_assignment` baru? — TBD, butuh DB query (`SHOW TABLES LIKE '%assignment%'`, `SELECT COUNT(*) FROM employee GROUP BY emp_id HAVING ... > 1`).
...
7. **Audit historis sender**: snapshot `employee` ke kolom JSON di `mail` (`m_sender_snapshot`) saat insert, vs reconstruct dari timestamp + audit log HR? — preferensi storage, tanya stakeholder.
